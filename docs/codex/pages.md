...
All pages are server-rendered PHP (no client-side routing, no SPA framework). "Client" below means JS that runs after load to add interactivity to the server-rendered HTML — there is no client-side data fetching except the two fetch() call sites noted. Every authenticated page is reachable from `includes/page_header.php` (mobile top nav), `includes/page_footer.php` (mobile bottom nav), and `includes/sidebar.php` (desktop sidebar ≥1024px, role-aware: renders `.parent-sidebar` or `.child-sidebar` markup depending on `$_SESSION['role']`). As of 2026-07-23, `includes/sidebar.php` is included by `dashboard_parent.php`, `dashboard_child.php`, `task.php`, `routine.php`, `goal.php`, `children.php`, and `rewards.php` (parent branch only — the child "Rewards Shop" branch is a separate distraction-free full-page experience with no header/nav/sidebar at all).
...
| `dashboard_parent.php` | Children overview, tasks/goals/rewards summaries, notifications, family management modals | inline `<head>` (does **not** use `includes/html_head.php`) | `css/main.css`, `css/parent.css` | `js/time-of-day.js`, `js/number-stepper.js` | Builds its own header/footer markup inline (does not use `includes/page_header.php`/`page_footer.php`), but wraps content in `.parent-page`/`.parent-main` and includes `includes/sidebar.php` for the desktop sidebar |
| `dashboard_child.php` | Hero stats, week strip, goals, quick-nav cards, notifications | inline `<head>` | `css/main.css`, `css/child.css` | `js/time-of-day.js`, `js/number-stepper.js` | Wraps content in `.child-page`/`.child-main` and includes `includes/sidebar.php` for the desktop sidebar |
| `children.php` | Per-child cards: level/streak/points/stars, history, week schedule modal | `includes/page_setup.php` + `includes/html_head.php` (the one page that follows the "shared partial" convention along with rewards.php) | `css/main.css`, `css/shared.css`, `css/children-detail.css` (via `$extraHeadCss`) | `js/child-detail.js` | Week-schedule fetch (`?week_schedule=1`) uses `serveWeekScheduleJson()`/`buildChildWeekSchedule()`, shared functions in `includes/functions.php` |
...
| `includes/sidebar.php` | most authenticated pages | Desktop sidebar nav (≥1024px); renders `.parent-sidebar` or `.child-sidebar` markup depending on `$_SESSION['role']` |
...
---
Last generated: 2026-07-23. Triggered by: initial /docs/codex reference generation request (grep of every root .php file's `<head>`, CSS/JS includes, and layout-partial usage); updated same day after fixing a bug report that the desktop sidebar didn't render on `dashboard_parent.php`/`dashboard_child.php`/`task.php`/`routine.php` and adding a child-role sidebar variant. Regenerate when a page's include pattern, CSS/JS dependencies, or the page tree itself changes.
