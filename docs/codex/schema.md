...
MySQL/MariaDB (InnoDB, utf8mb4). No ORM — raw PDO + prepared statements. Schema is **not** managed by migration files in the traditional sense: `includes/functions.php` runs an idempotent bootstrap on *every request* (`CREATE TABLE IF NOT EXISTS` + `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` for every table/column) near the bottom of the file (~line 4740–5380). `schema_migrations` tracks one named, one-time migration (`preset_tasks_v1`, see below). Source of truth for exact current DDL = that bootstrap block in `includes/functions.php`, not a schema.sql file. `tests/fixtures/legacy_schema.sql` is a frozen **pre-migration** snapshot used only as a test fixture (old names `routine_tasks` / `routines_routine_tasks`).
